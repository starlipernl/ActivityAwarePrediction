...
for test_section_day = 3%section_days
    ...
    for cluster = 1:cluster_count
        ...
        %trainRegressionModel = str2func(['trainRegressionModel_d' num2str(test_section_day) '_c' num2str(cluster), '_FL' num2str(FL)]);
        ...
        svm_predicted_HR{test_section_day}{cluster} = trainedModel.predictFcn(test_features_Z_optimal);
        svm_smooth_predicted_HR{test_section_day}{cluster} = movmean(svm_predicted_HR{test_section_day}{cluster}, [4 0]);
        svm_actual_HR{test_section_day}{cluster} = test_targetHR;
...
%     svm_avg_RMSE{test_section_day} = 0;
%     for cluster = 1:cluster_count
%         svm_avg_RMSE{test_section_day} = svm_avg_RMSE{test_section_day} + svm_RMSE{test_section_day}{cluster} * ...
%             sum(cluster_inds{test_section_day}{test_section_day} == cluster);
%     end
%     svm_avg_RMSE{test_section_day} = svm_avg_RMSE{test_section_day} / length(cluster_inds{test_section_day}{test_section_day});
% end
