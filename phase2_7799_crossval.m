...
%         [CV_MSE_HR,CV_RMSE_HR,CV_NRMSE_HR]  = deal(zeros(size(CV_actualHR)));
        ...
        for lambdaInd = 1:length(lambdas)
            ...
            CV_predicted_HR = (CV_test_features_Z_orth * CV_b_grp_HR(:, lambdaInd)) * CVtargetHR_sigma + CVtargetHR_mu; %+ CV_test_features(:,52);
...
for cluster = 1:clusterNum
    ...
    for lambdaInd = 1:length(lambdas)
        ...
        predicted_HR{cluster}(:,lambdaInd) = (test_features_Z_orth * b_grp_HR) * targetHR_sigma + targetHR_mu;
