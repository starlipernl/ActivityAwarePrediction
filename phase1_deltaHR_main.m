% Main power optimization code to predict change in heart rate
% Contains sections for loading data, reducing dimensionality, 
% clustering, cross-validation, visualizations
...
working_dir = pwd;
dayPostData = {
    [working_dir '\Data\y18m02d14\Post\DataFeatures.mat'] % Day 1
    [working_dir '\Data\y18m02d16\Post\DataFeatures.mat'] % Day 2
    [working_dir '\Data\y18m03d01_2\Post\DataFeatures.mat'] % Day 3
    [working_dir '\Data\y18m03d01\Post\DataFeatures.mat'] % Day 4
    };
...
res_cp.rms = rms(target_HR{testDayInd});
res_cp.nrms = res_cp.rms/range(actual_testHR);
res_cp.rmsr = rms(target_HR{testDayInd}./actual_testHR);
res_cp.mae = sum(abs(target_HR{testDayInd}))/length(actual_testHR);
res_cp.mape = sum(abs(target_HR{testDayInd}./actual_testHR))/length(actual_testHR);
res_cp.r2 = 1 - sum(target_HR{testDayInd}.^2)/sum((actual_testHR-mean(actual_testHR)).^2);
...
%     h = subplot(2, 3, cluster);
    ...
    feature_lost_table = feature_lost_table([1 2 3 4 25 46 52], :);
...
figure;
time = 1:length(cluster_inds{testDayInd}{testDayInd});
...
set(gca, 'fontsize', 10)
hold off
h2 = subplot(3,1,2); plot(time/60, cluster_mode, '.', 'MarkerSize', 10); 
...
for ii = 1:3
plot(features_Z_reduced{testDayInd}(cluster_inds_plot{cluster}, ii)); 
%legend('IMU Feature')
end
hold off
ylabel('Accel');
...
for test_section_day = test_section_days
    for cluster = 1:cluster_count
        increaseHRAmountLambdaInd{test_section_day}{cluster}(1) = optimal_lambda{test_section_day}{cluster};
        for increaseAmountInd = 2:length(increaseAmounts)
            ...
            if increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd-1) == 300
                increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd) = 300;
            elseif any(sqrt(avg_CV_MSE_HR{test_section_day}{cluster}(increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd-1):end))<=sqrt(avg_CV_MSE_HR{test_section_day}{cluster}(increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd-1))))
%                 [~, increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd)] = min(...
%                         (sqrt(avg_CV_MSE_HR{test_section_day}{cluster}(increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd-1):end)) - ...
%                         (sqrt(avg_CV_MSE_HR{test_section_day}{cluster}(optimal_lambda{test_section_day}{cluster}))+increaseAmount)));
                [~, increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd)] = find(...
                        (sqrt(avg_CV_MSE_HR{test_section_day}{cluster}(increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd-1):end))<= ...
                        (sqrt(avg_CV_MSE_HR{test_section_day}{cluster}(optimal_lambda{test_section_day}{cluster}))+increaseAmount)), 1, 'last');
                increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd) = ...
                        increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd) + increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd-1)-1;
            else
                [~, increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd)] = min(...
                    abs(sqrt(avg_CV_MSE_HR{test_section_day}{cluster}(increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd-1):end)) - ...
                    (sqrt(avg_CV_MSE_HR{test_section_day}{cluster}(optimal_lambda{test_section_day}{cluster}))+increaseAmount)));
                increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd) = ...
                    increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd) + increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd-1);
    %             [~, increaseHRAmountLambdaInd{test_section_day}{cluster}(increaseAmountInd)] = min(...
