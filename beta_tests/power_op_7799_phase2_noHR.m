...
featureGroupIDs = [1 2 3*ones(1,21) 4*ones(1,21)]; 
groupNames = {'Env. Temp', 'Humidity', 'Wrist accel', 'Chest accel'};
...
lambdas = exp(linspace(log(0.1), log(100), 500)); %group lasso parameter
...
%         CVvalidTarget, cvvalid_cluster_inds{fold}, delh);
    ...
    for cluster = 1:clusterNum
        ...
        clust_train_features_Z = cvtrain_features_Z(cvtrain_cluster_inds{fold} == cluster, :);
        clust_train_targetHR_Z = cvtrain_targetHR_Z(cvtrain_cluster_inds{fold} == cluster, :);
        clust_valid_features_Z = cvvalid_features_Z(cvvalid_cluster_inds{fold} == cluster, :);
        clust_valid_targetHR = CVvalidTarget(cvvalid_cluster_inds{fold} == cluster, :);
        ...
        CV_nump_cluster = sum(cvvalid_cluster_inds{fold} == cluster);
        CV_nump = length(cvvalid_cluster_inds{fold});
...
for cluster = 1:clusterNum
    ...
    for fold = 1:numFolds
        avg_CV_MSE_HR{cluster} = avg_CV_MSE_HR{cluster} +  ...
            ...
            (sum(cvvalid_cluster_inds{fold} == cluster) / ...
            length(cvvalid_cluster_inds{fold}));
...
train_cluster_inds = train_cluster_inds_noshift;
cluster_inds = cluster_inds_noshift;
% shift data by delh
...
% normalize test data with training transform parameters
test_targetHR_Z = bsxfun(@rdivide, bsxfun(@minus, test_features, targetHR_mu), targetHR_sigma);
...
for cluster = 1:clusterNum
    ...
    test_targetc = test_target(cluster_inds == cluster, :);
...
avg_NRMSE_HR = avg_RMSE_HR/range(test_target);
...
res_cp.err = actual_testHR - mean(test_target);
