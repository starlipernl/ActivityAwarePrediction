% Load reduced features set and clustering indices and intialize vars
clc; clear;
load('features_reduced_all_new');
clusterNum = 6; %number of clusters (activities)
...
currentSensor = [0.312 0.263, 0.263, 0.312, 0.263];
...
for fold = 1:numFolds
    ...
    CVtrainTarget_shift = CVtrainTarget_shift - CVtrainFeats_shift(:,indHR);
    CVvalidTarget_shift = CVvalidTarget_shift - CVvalidFeats_shift(:,indHR);
    if clusterNum == 1
        cvtrain_c_inds_shift{fold}(:,1) = 1;
        cvvalid_c_inds_shift{fold}(:,1) = 1;        
    end
    % normalize training data 
    [cvtrain_features_Z, CVfeatures_mu, CVfeatures_sigma] = zscore(CVtrainFeats_shift);
    [cvtrain_targetHR_Z, CVtargetHR_mu, CVtargetHR_sigma] = zscore(CVtrainTarget_shift);
    % normalize test data with training transform parameters
    cvvalid_targetHR_Z = bsxfun(@rdivide, bsxfun(@minus, CVvalidTarget_shift, CVtargetHR_mu), CVtargetHR_sigma);
    cvvalid_features_Z = bsxfun(@rdivide, bsxfun(@minus, CVvalidFeats_shift, CVfeatures_mu), CVfeatures_sigma);
            
    for cluster = 1:clusterNum
        %Extract features for train and valid days for specific cluster
        clust_train_features_Z = cvtrain_features_Z(cvtrain_c_inds_shift{fold} == cluster, :);
        clust_train_targetHR_Z = cvtrain_targetHR_Z(cvtrain_c_inds_shift{fold} == cluster, :);
        clust_valid_features_Z = cvvalid_features_Z(cvvalid_c_inds_shift{fold} == cluster, :);
        clust_valid_targetHR = CVvalidTarget_shift(cvvalid_c_inds_shift{fold} == cluster, :);
        clust_valid_featsHR = CVvalidFeats_shift(cvvalid_c_inds_shift{fold} == cluster, indHR);
        ...
        CV_actualHR = clust_valid_targetHR + clust_valid_featsHR;
        CV_nump_cluster = sum(cvvalid_c_inds_shift{fold} == cluster);
        CV_nump = length(cvvalid_c_inds_shift{fold});
%         [CV_MSE_HR,CV_RMSE_HR,CV_NRMSE_HR]  = deal(zeros(size(CV_actualHR)));
        ...
        for lambdaInd = 1:length(lambdas)
            ...
            CV_predicted_HR = (CV_test_features_Z_orth * CV_b_grp_HR(:, lambdaInd)) * CVtargetHR_sigma + CVtargetHR_mu + clust_valid_featsHR;
...
if clusterNum == 1
    train_cluster_inds(:) = 1;
    cluster_inds(:) = 1;        
end
train_target_shift = train_target_shift - train_feats_shift(:,indHR);
test_target_shift = test_target_shift - test_feats_shift(:,indHR);
% normalize training data 
[train_features_Z, features_mu, features_sigma] = zscore(train_feats_shift);
[train_targetHR_Z, targetHR_mu, targetHR_sigma] = zscore(train_target_shift);
% normalize test data with training transform parameters
test_targetHR_Z = bsxfun(@rdivide, bsxfun(@minus, test_target_shift, targetHR_mu), targetHR_sigma);
test_features_Z = bsxfun(@rdivide, bsxfun(@minus, test_feats_shift, features_mu), features_sigma);


%% Test models 
tic
for cluster = 1:clusterNum
    train_features_Zc = train_features_Z(train_cluster_inds == cluster, :);
    train_targetHR_Zc = train_targetHR_Z(train_cluster_inds == cluster, :);
    test_targetc = test_target_shift(cluster_inds == cluster, :);
    test_featsHRc = test_feats_shift(cluster_inds == cluster, indHR);
    test_features_Zc{cluster} = test_features_Z(cluster_inds == cluster, :);
    ...
    for lambdaInd = 1:length(lambdas)
        ...
        predicted_HR{cluster}(:,lambdaInd) = (test_features_Z_orth * b_grp_HR) * targetHR_sigma + targetHR_mu + test_featsHRc;
        %smooth_CV_predicted_BR = movmean(predicted_BR, [4 0]);
        actualHR = test_targetc + test_featsHRc;
...
actual_testHR = test_target_shift + test_feats_shift(:,indHR);
...
res_cp.err = actual_testHR - test_feats_shift(:,indHR);
...
for cluster = 1:clusterNum
    ...
    for lambdaInd = 1:length(lambdas)
        for groupInd = groupInds(sensorGroupsUsed{cluster}(lambdaInd, 1:5))
    ...
    optimalsensorGroupsUsed{cluster} = sensorGroupsUsed{cluster}(lambdaInd, 1:5);
...
%     h = subplot(2, 3, cluster);
    ...
    feature_lost_table = feature_lost_table([1 2 3 24 45], :);
